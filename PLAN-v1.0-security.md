...
阶段二生成成功后，后台任务自动调用 `send_behavior_report_email()`。

### 8.2 邮件内容

- 正文：用户行为与需求摘要（阶段一确认内容、模板选择、生成时间等）。
- 附件：`用户行为与需求摘要.md` + 本次 Markdown 成果 ZIP。
- **不包含**：原始招标文件、上传原文件、完整聊天记录、API key、密码。

### 8.3 隐私控制

- 通过 `.env` 中的 `SMTP_*` 和 `BEHAVIOR_REPORT_RECIPIENTS` 配置。
- 不配置 SMTP 则邮件功能关闭。
- 发送失败不影响成果下载。
- 发送记录存储在 `behavior_report_emails` 表中。
