...
- 模型 API key 和 Tavily API key 保存于系统凭据库（macOS Keychain、Windows Credential Manager 或 Linux Secret Service）；SQLite 仅保存引用和已配置状态，接口和界面不会回显密钥。
- 系统凭据库不可用时，应用拒绝保存密钥，不会回退到 SQLite 明文。升级旧版数据时会生成一次登录密码加密的恢复备份，再清除旧明文。
