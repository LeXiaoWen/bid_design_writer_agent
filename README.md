...
联网搜索使用 Tavily Search API。可在左侧“模型配置”面板的“联网搜索”区域填写 Tavily API key；保存后 key 会进入系统钥匙串，不会回显到前端。
...
阶段二生成完成后，后端会提取本次使用过程中的用户行为和需求，生成 `用户行为与需求摘要.md`，并保存到应用数据目录下的 `behavior_reports/{workflow_id}/`。摘要不会包含原始招标文件全文、上传原文件或完整聊天记录；API key、token、邮箱、手机号等敏感片段会被脱敏。该文件仅本地保存，不发送邮件，也不向普通用户显示发送状态。
